...
class DeleteReservationRepositoryTest {
    ...
    private lateinit var mockRetrofit: APIDeleteReservation
    ...
    fun setUp() {
        ...
        mockDeleteReservationRepository = Mockito.spy(DeleteReservationRepository(mockNetworkClient))
        mockRetrofit = mock()
    ...
    fun changePassword_correct() {
        Mockito.doReturn(mockRetrofit).`when`(mockNetworkClient).buildService(APIDeleteReservation::class.java)
        Mockito.doReturn(mockCall).`when`(mockRetrofit).deleteReservation("authorization", "stanza1")
        ...
        mockDeleteReservationRepository.deleteReservation("stanza1", "authorization")
        assertTrue(mockDeleteReservationRepository.serverResponse.value?.peekContent() is Result.Success)
    }
    ...
    fun changePassword_error() {
        ...
        mockDeleteReservationRepository.deleteReservation("stanza1", "authorization")
        assertTrue(mockDeleteReservationRepository.serverResponse.value?.peekContent() == Result.Error(exception = "Internal Server Error"))
    }
    ...
    fun changePassword_exception() {
        ...
        mockDeleteReservationRepository.deleteReservation("stanza1", "authorization")
        assertTrue(mockDeleteReservationRepository.serverResponse.value?.peekContent() == Result.Error(exception = "Timeout"))
    }
}
